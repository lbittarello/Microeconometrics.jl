...

{
    ...
    "category": "section",
    "text": "This package provides support for microeconometric estimation. It supports complex covariance structures (clustered, etc.) and weighted data. Please report bugs by opening an issue. Information on specific versions can be found on the release page."
},
...

{
    ...
    "title": "CorrStructure",
    "category": "section",
    "text": "This structure specifies the correlation between observations. It determines the calculation of standard errors.All constructors accept the Boolean keyword adj, which defaults to true. If true, a finite-sample adjustment is applied to the variance matrix. The adjustment factor is n / (n - 1), where n is the number of clusters for clustered data and the number of observations otherwise.Four subtypes are currently available:"
},
...

{
    ...
    "category": "section",
    "text": "This structure accommodates other correlation structures. The first argument determines the precise pattern. The following methods are available:CrossCorrelated(\"Two-way clustering\", DF::DataFrame, cluster₁::Symbol, cluster₂::Symbol; adj::Bool = true)Observations may be arbitrarily correlated if they share any cluster.CrossCorrelated(\"Time\",\n    DF::DataFrame,\n    time::Symbol,\n    bandwidth::Real,\n    [kernel::Function = parzen];\n    adj::Bool = true)The maximum possible correlation between two observations declines with the time difference between them. Correlation is arbitrary below that limit. The bandwidth and the kernel function control the upper bound. time specifies the column of DF that contains the date of each observation (Date).The following kernels are predefined for convenience: Bartlett (bartlett), Parzen (parzen), Truncated (truncated) and Tukey-Hanning (tukeyhanning). See Andrews (1991) for formulae.CrossCorrelated(\"Space\",\n    DF::DataFrame,\n    latitude::Symbol,\n    longitude::Symbol,\n    bandwidth::Real,\n    [kernel::Function = parzen];\n    adj::Bool = true)The maximum possible correlation between two observations declines with the spatial distance between them. Correlation is arbitrary below that limit. The bandwidth and the kernel function control the upper bound. latitude and longitude specify the columns of DF that contain the coordinates of each observation in radians (Float64).For a list of predefined kernels, see CrossCorrelated(\"time\", args...) above.CrossCorrelated(\"Time and space\",\n    DF::DataFrame,\n    time::Symbol,\n    bandwidth_time::Real,\n    latitude::Symbol,\n    longitude::Symbol,\n    bandwidth_space::Real,\n    [kernel::Function = parzen];\n    adj::Bool = true)The maximum possible correlation between two observations declines with the time difference and the spatial distance between them. Correlation is arbitrary below that limit. The bandwidths and the kernel function control the upper bound. time specifies the column of DF that contains the date of each observation. latitude and longitude specify the columns of DF that contain the coordinates of each observation in radians (Float64).For a list of predefined kernels, see CrossCorrelated(\"time\", args...) above."
},
...

{
    ...
    "title": "Estimators",
    "category": "section",
    "text": "The function fit estimates models. It returns a model structure, which contains the estimation sample, coefficients and their covariance matrix (inter alia). For example, the output of fit(OLS, MD) has type OLS. Some have additional fields: e.g., two-stage models carry estimates from the first stage. These structures are instances of broader abstract types, such as MLE or TwoStageModel, which belong to the supertype Micromodel in their turn.If you only need coefficients, set novar = true."
},
...

{
    ...
    "category": "section",
    "text": "fit(OLS, MD::Microdata; novar::Bool = false)The Microdata must contain: response and control. OLS is a subtype of ParModel. It supports predict and fitted."
},

{
    ...
    "title": "Binary choice",
    "category": "section",
    "text": "fit(Logit, MD::Microdata; novar::Bool = false)The Microdata must contain: response and control. Logit is a subtype of MLE and ParModel. It supports predict and fitted.fit(Probit, MD::Microdata; novar::Bool = false)The Microdata must contain: response and control. Probit is a subtype of MLE and ParModel. It supports predict and fitted."
},
...

{
    ...
    "category": "section",
    "text": "This package supports the methods for regression models of StatsBase.The following functions are available for all models: nobs and model_response.The following functions are available for parametric models: , dof, dof_residual, coef, stderr, vcov, confint, coefnames and coeftable. Three keywords customize the behavior of coeftable: verbose (Boolean: suppresses printing), digits (integer: controls rounding) and level (float: controls the level of the confidence interval – 0.0 for none). Note that all methods refer to the second stage of two-stage models.The following functions are available for maximum-likelihood estimators: aic, aicc, bic, deviance, nulldeviance, loglikelihood, nullloglikelihood, r2 and adjr2. There are also R² methods for OLS and IV.Some models support predict and fitted (see the documentation). predict estimates the index of single-index models. fitted estimates the conditional outcome expectation. For example, predict estimates the Xβ of a logit model, whereas fitted estimates logistic(Xβ). Support for residuals depends on the availability of fitted. Out-of-sample forecast is planned for a future release."
},

{
    "location": "methods.html#Additional-methods-1",
    "page": "Methods",
    "title": "Additional methods",
    "category": "section",
    "text": "tstat: the t-statistic (i.e., the ratio of coefficients to standard error);\npval: the p-value of a two-sided significance test;\nfirst_stage: the first-stage estimates of a two-stage model."
},

{
    ...
    "title": "Hausman test",
    "category": "section",
    "text": "These functions compute the difference in coefficients between two parametric models. They return a ParObject, which contains the vector of differences, their covariance matrix and labels. Our implementation is based on the GMM representation of the joint estimation problem (see Subsection 8.3.2 of Cameron and Trivedi (2005)).hausman_1s(\n    model₁::Union{ParModel, TwoStageModel},\n    model₂::Union{ParModel, TwoStageModel},\n    names::Vector{String} = intersect(coefnames(obj₁), coefnames(obj₂)))This function is appropriate when model₁ and model₂ were both based on the same estimation sample.hausman_2s(\n    model₁::Union{ParModel, TwoStageModel},\n    model₂::Union{ParModel, TwoStageModel},\n    names::Vector{String} = intersect(coefnames(obj₁), coefnames(obj₂)))This function is appropriate when model₁ and model₂ were based on independent samples. For example, the samples might consist of independent observations with no overlap.hausman_2s(\n    model₁::Union{ParModel, TwoStageModel},\n    model₂::Union{ParModel, TwoStageModel},\n    corr::CorrStructure,\n    names::Vector{String} = intersect(coefnames(obj₁), coefnames(obj₂)))This function is appropriate when model₁ and model₂ were based on dependent samples. For example, the samples might consist of independent observations with some overlap or clustered observations with shared clusters. The correlation structure corr must specify the correlation between all observations of both estimation samples. For example, you could construct corr for the entire dataset and construct the samples via the subset keyword to Microdata."
},
