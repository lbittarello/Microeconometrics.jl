...

{
    ...
    "title": "Installation",
    "category": "section",
    "text": "To install the package, run:julia> Pkg.add(\"Microeconometrics\")To update to the latest release, run:julia> Pkg.update(\"Microeconometrics\")To obtain the last version, run:julia> Pkg.checkout(\"Microeconometrics\")"
},
...

{
    ...
    "title": "Model specification",
    "category": "section",
    "text": "Microdata combines the functionalities of ModelFrame and ModelMatrix from StatsModels.jl. It contains a Matrix{Float64} (the data matrix), a map from model components to matrix columns, a correlation structure and weights (inter alia).Microdata(\n        DF::DataFrame,\n        model::Dict{Symbol, String};\n        contrasts::Dict,\n        subset::AbstractVector{Bool} = trues(size(DF, 1)),\n        vcov::CorrStructure = Heteroscedastic(),\n        weights::AbstractWeights = UnitWeights(size(DF, 1))\n    )To construct a Microdata, two arguments are compulsory: a DataFrame, which contains the data, and a dictionary, which specifies the components of the model of interest.All regression models need a response, but other requirements may vary. For example, OLS asks for response and control. See the tutorial for examples. You pass these sets as strings, following syntax of Formula. Conventional sets include:response: the response (a.k.a. outcome or dependent variable);\ncontrol: exogenous explanatory variables (n.b.: you must explicitly include intercepts, + 1);\ntreatment: endogenous explanatory variables;\ninstrument: instrumental variables (i.e. excluded exogenous variables).As for the keywords:contrasts: a dictionary from column labels to contrast schemes.\nsubset determines the estimation sample. Set a row to true if the corresponding row of DF should be included and false if it should be excluded. This keyword is useful if you are comparing subgroup effects and observations in different subgroups may correlate(e.g., they may belong to the same cluster). hausman_2s will account for that correlation if the Microdata were constructed with subset.weights is a weight vector. Except for frequency weights, the weight vector is normalized to sum up to the number of observations in the sample.\nvcov is a correlation structure.It is also possible to base new Microdata on existing Microdata:Microdata(MD::Microdata; kwargs...)This constructor allows you to reassign variables to new sets. You can also create new variable sets. If you do not redefine a set, it is preserved. To suppress a set, redefine it to \"\". You cannot add new variables to the data matrix, modify the correlation structure, restrict the sample or reweight observations.This functionality is useful if you wish to compare specifications. Rather than building separate data matrices for each one of them, you can build a master Microdata, holding all variables of interest, and adjust its map as you go through specifications."
},
...

{
    ...
    "category": "section",
    "text": "fit(OLS, MD::Microdata)The Microdata must contain: response and control. OLS is a subtype of ParModel."
},
...

{
    ...
    "title": "Binary choice",
    "category": "section",
    "text": "fit(Logit, MD::Microdata)\nfit(Probit, MD::Microdata)\nfit(Cloglog, MD::Microdata)The Microdata must contain: response and control. The model structures are subtypes of MLE."
},
...
{
    "location": "methods.html#Methods-from-*StatsBase.jl*-1",
    "page": "Methods",
    "title": "Methods from StatsBase.jl",
    "category": "section",
    "text": "This package supports the methods for regression models of StatsBase.jl.The following functions are available for all models: nobs and model_response.The following functions are available for parametric models: dof, dof_residual, coef, stderr, vcov, confint, coefnames and coeftable. Two keywords customize the behavior of coeftable: digits (integer: controls rounding) and level (float: controls the level of the confidence interval – 0.0 for none). Note that all methods refer to the second stage of two-stage models.The following functions are available for maximum-likelihood estimators: aic, aicc, bic, deviance, nulldeviance, loglikelihood, nullloglikelihood, r2 and adjr2. There are also R² methods for OLS and IV.Most models support predict and fitted. predict estimates the index of single-index models. fitted computes the conditional outcome expectation. For example, predict estimates the Xβ of a logit model and fitted computes logistic(Xβ). Support for residuals depends on the availability of fitted. Out-of-sample forecast is supported."
},
